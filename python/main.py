...
class ResumeHandler(FileSystemEventHandler):
    def __init__(self, json_path, html_template_path, output_html_path):
        ...
        self.env = Environment(
            loader=FileSystemLoader(os.path.dirname(html_template_path)),
    ...
    def on_modified(self, event):
        if event.src_path == self.json_path:
            ...
            print(
                f"Detected changes in {self.html_template_path}, reloading template..."
            )
    ...
    def render_template(self):
        try:
            ...
            print(
                f"You can view your rendered resume at:  file://{self.output_html_path}"
            )
        except Exception as e:
...
class ResumeWatcher:
    def __init__(self, json_path, html_template_path, output_html_path):
        ...
        self.event_handler = ResumeHandler(
            json_path, html_template_path, output_html_path
        )
        self.observer = Observer()

    def start(self):
        self.observer.schedule(
            self.event_handler, path=os.path.dirname(self.json_path), recursive=False
        )
        self.observer.schedule(
            self.event_handler,
            path=os.path.dirname(self.html_template_path),
            recursive=False,
        )
        self.observer.start()
        print(
            f"Watching for changes in {self.json_path} and {self.html_template_path}..."
        )
        try:
            while True:
